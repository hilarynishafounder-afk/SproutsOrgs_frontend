              ...
              <div style={{ display: 'flex', gap: 4, marginTop: 4 }}>
                {(Array.isArray(h.images) ? h.images : (typeof h.images === 'string' ? h.images.split(',').filter(Boolean) : [])).slice(0, 3).map((img, idx) => (
                  <div key={idx} style={{ width: 24, height: 24, borderRadius: 4, overflow: 'hidden', border: '1px solid var(--border-gray)' }}>
                    <img src={img.startsWith('http') ? img : `https://sproutsorgs-backend-8xrf.onrender.com${img}`} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  </div>
                ))}
    ...
    <div className="image-input-container">
      ...
      {preview && (
        <div className="image-preview" style={{ height: 'auto', maxHeight: '300px' }}>
          <img src={preview.startsWith('data:') || preview.startsWith('http') ? preview : `https://sproutsorgs-backend-8xrf.onrender.com${preview}`} alt="Preview" style={{ width: '100%', height: 'auto', objectFit: 'contain' }} />
        </div>
      )}
    </div>
  );
...
function MultiImageInput({ existingImages, onRemoveExisting, onFilesChange }) {
  ...
  const getImageUrl = (url) => {
    ...
    return `https://sproutsorgs-backend-8xrf.onrender.com${url}`;
  };
